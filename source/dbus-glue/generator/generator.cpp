...
namespace DBusGlue
{
    ...
    //---------------------------------------------------------------------------------------------------------------------
    std::string
    interface_generator::get_introspected_xml_from(dbus& bus, std::string const& service, std::string const& path)
    {
        auto interface = create_interface<DBusGlue::detail::IIntrospectable>(
            bus, service, path, "org.freedesktop.DBus.Introspectable");
